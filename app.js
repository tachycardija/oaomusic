...
function goTo(id, isBack = false) {
  ...
  if (cur && !isBack) {
    navHistory.push(cur.id);   // запоминаем откуда пришли
  }
...
function goBack() {
  const prev = navHistory.pop();          // берём последнюю страницу из истории
  if (!prev) return;
  // если возвращаемся на page2 — обновляем плейлисты
  if (prev === "page2") { updateFavCount(); renderCustomPlaylistBlocks(); }
  document.querySelectorAll(".page").forEach(p => p.classList.remove("active", "back-in"));
  const next = document.getElementById(prev);
  next.classList.add("active");
  next.classList.add("back-in");
}
...
document.getElementById("backToSearch").addEventListener("click",    goBack);
document.getElementById("backToPlaylists").addEventListener("click", goBack);
document.getElementById("backFromArtist").addEventListener("click",  goBack);
...
function renderTracks(tracks, container, opts = {}) {
  ...
  tracks.forEach(track => {
    ...
    el.querySelector(".icon-btn--send").addEventListener("click", () => {
      ...
      showToast("Отправлено боту — жди трек в чате");
    });
...
// Свайп-закрытие на всех шторках
makeSwipeable(document.getElementById("addSheet"),      closeSheet);
makeSwipeable(document.getElementById("fabSheet"),      closeFabSheet);
makeSwipeable(document.getElementById("createSheet"),   closeCreateSheet);
makeSwipeable(document.getElementById("importSheet"),   closeImportSheet);
...
// Spotify — отправляем ссылку боту
document.getElementById("importSpotifyBtn").addEventListener("click", () => {
  const url = document.getElementById("importSpotifyUrl").value.trim();
  const res = document.getElementById("importSpotifyResult");
  if (!url || !url.includes("spotify.com")) {
    showToast("Вставь ссылку на плейлист Spotify");
    return;
  }
  const tg = window.Telegram?.WebApp;
  if (!tg) { showToast("Только в Telegram"); return; }
  tg.sendData(JSON.stringify({ action: "import_playlist_url", source: "spotify", url }));
  res.innerHTML = `<div class="msg-empty" style="color:var(--accent)">✅ Отправлено боту — жди треки в чате</div>`;
  showToast("Бот переносит плейлист…");
});

// SoundCloud — отправляем ссылку боту
document.getElementById("importScBtn").addEventListener("click", () => {
  ...
  if (!url || !url.includes("soundcloud.com")) {
    showToast("Вставь ссылку на плейлист SoundCloud");
    return;
  }
  const tg = window.Telegram?.WebApp;
  if (!tg) { showToast("Только в Telegram"); return; }
  tg.sendData(JSON.stringify({ action: "import_playlist_url", source: "soundcloud", url }));
  res.innerHTML = `<div class="msg-empty" style="color:var(--accent)">✅ Отправлено боту — жди треки в чате</div>`;
  showToast("Бот переносит плейлист…");
});
...
════════════════════════════════════════ */
// Избранные — аккордеон (вся строка кликабельна кроме кнопки)
document.getElementById("plFav").querySelector(".pl-block__row").addEventListener("click", (e) => {
...
// Скачать избранные
document.getElementById("favDownloadBtn").addEventListener("click", (e) => {
  e.stopPropagation();
  const tg = window.Telegram?.WebApp;
  const favs = getFavs();
  if (!favs.length) { showToast("Нет избранных треков"); return; }
  if (!tg) { showToast("Только в Telegram"); return; }
  tg.sendData(JSON.stringify({
    action: "request_playlist",
    name: "Избранные",
    tracks: favs.map(t => ({ title: t.title, artist: t.artist?.name || t.artist || "" }))
  }));
  showToast("Отправлено боту — жди треки в чате");
});
...
function renderCustomPlaylistBlocks() {
  ...
  pls.forEach((p, i) => {
    ...
    block.innerHTML = `
      <div class="pl-block__row" style="cursor:pointer">
        ...
        <button class="icon-btn" style="color:var(--accent)" data-dl="${i}" aria-label="Скачать плейлист">
          <svg viewBox="0 0 24 24"><polyline points="8,17 12,21 16,17"/><line x1="12" y1="21" x2="12" y2="7"/><line x1="4" y1="4" x2="20" y2="4"/></svg>
        </button>
    ...
    // Раскрыть — вся строка кликабельна кроме кнопок
    ...
    block.querySelector(".pl-block__row").addEventListener("click", (e) => {
      if (e.target.closest("button")) return; // не реагируем на кнопки
      toggleBlock();
    });
    block.querySelector(".pl-block__chev").addEventListener("click", toggleBlock);

    // Скачать плейлист
    block.querySelector("[data-dl]").addEventListener("click", (e) => {
      e.stopPropagation();
      const tg = window.Telegram?.WebApp;
      const tracks = getPls()[i]?.tracks || [];
      ...
      tg.sendData(JSON.stringify({
        action: "request_playlist",
        name: getPls()[i].name,
        tracks: tracks.map(t => ({ title: t.title, artist: t.artist?.name || t.artist || "" }))
      }));
      showToast(`Отправлено боту — жди треки в чате`);
    });
...
════════════════════════════════════════ */
function makeSwipeable(sheetEl, closeFn) {
  let startY = 0, curY = 0, dragging = false;

  function onStart(e) {
    const touch = e.touches ? e.touches[0] : e;
    // только если тащим сверху 60px шторки (хэндл зона)
  ...
  function onMove(e) {
    if (!dragging) return;
    const touch = e.touches ? e.touches[0] : e;
    curY = Math.max(0, touch.clientY - startY);
